...
async function getLevelInfo(xp) {
...
async function awardXP(userId, xpAmount) {
  ...
  await dbUpdate('usuarios', userId, { xp_total: newXP, nivel: newLevel.nivel });
...
async function checkMedals(userId) {
  const user     = (await dbFindAll('usuarios')).find(u => u.id === Number(userId));
  if (!user) return [];

  const configs  = await dbFindAll('medalhas_config');
  const already  = await dbFindWhere('medalhas_aluno', m => m.aluno_id === Number(userId)).map(m => m.medalha_id);
  const respostas = (await dbFindAll('respostas')).filter(r => r.aluno_id === Number(userId));
  const tentativas = (await dbFindAll('tentativas')).filter(t => t.aluno_id === Number(userId) && t.status === 'concluida');
  ...
  for (const cfg of configs) {
    ...
    switch (cfg.criterio) {
      ...
      case 'trilha_completa':
        ...
        const trilhas = await dbFindAll('trilhas');
        const questoesMap = {};
        for (const t of trilhas) {
          const qs = (await dbFindAll('questoes')).filter(q => q.trilha_id === t.id && q.ativo !== false);
    ...
    if (ganhou) {
      await dbInsert('medalhas_aluno', { aluno_id: Number(userId), medalha_id: cfg.id, conquistada_em: new Date().toISOString() });
...
async function getGamificationProfile(userId) {
  ...
  const medalhasAluno = await dbFindWhere('medalhas_aluno', m => m.aluno_id === Number(userId));
  const medalhasConfig = await dbFindAll('medalhas_config');
  const medalhas = medalhasAluno.map(m => {
      const cfg = medalhasConfig.find(c => c.id === m.medalha_id);
      return cfg ? { ...cfg, conquistada_em: m.conquistada_em } : null;
    }).filter(Boolean);

  // Ranking position
  const todos = (await dbFindAll('usuarios')).filter(u => u.perfil === 'aluno' && u.status === 'ativo');
  ...
  return {
    ...
    medalhas,
    total_medalhas_disponiveis: await dbFindAll('medalhas_config').length,
  };
}
...
async function updateStreak(userId) {
  ...
  if (ultima !== hoje) {
    ...
    await dbUpdate('usuarios', userId, { streak_atual: streak, streak_maximo: streakMax, ultima_atividade: new Date().toISOString() });
  }
  return streak;
}
