...
async function checkAndCalibrate(questaoId) {
  const respostas = await dbFindWhere('respostas', r => r.questao_id === Number(questaoId));
  if (respostas.length < MIN_RESPOSTAS) return null;

  const questao = await dbFindWhere('questoes', q => q.id === Number(questaoId))[0];
  ...
  await dbUpdate('questoes', questaoId, { tri: triAtualizado });
...
async function runBatchCalibration() {
  const questoes = await dbFindWhere('questoes', q => q.ativo);
  let calibradas = 0;
