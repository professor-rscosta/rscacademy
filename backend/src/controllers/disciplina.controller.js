...
async function list(req, res, next) {
  try {
    ...
    if (req.user.perfil === 'aluno') {
      ...
      disciplinas = await repo.findByProfessor(req.user.id);
    } else {
      disciplinas = await repo.findAll();
    }
...
async function getById(req, res, next) {
  try {
    ...
    if (req.user.perfil === 'aluno') {
      ...
      const meusDiscIds = await adRepo.disciplinaIds(req.user.id);
    ...
    const turmasVinculadas = await tdRepo.findByDisciplina(d.id);
...
async function create(req, res, next) {
  try {
    ...
    const d = await repo.create({ nome, descricao, codigo, carga_horaria: carga_horaria||60, professor_id: profId });
...
async function update(req, res, next) {
  try {
    const d = await repo.update(req.params.id, req.body);
    if (!d) return res.status(404).json({ error: 'Disciplina não encontrada.' });
...
async function remove(req, res, next) {
  try {
    ...
    const _tds = await tdRepo.findByDisciplina(req.params.id);
    for (const td of _tds) await tdRepo.desvincular(td.turma_id, req.params.id);
...
async function getModulo(req, res, next) {
  try {
    ...
    const d = await repo.findById(discId);
    if (!d) return res.status(404).json({ error: 'Disciplina não encontrada.' });
    ...
    // Materiais por tipo
    const todosMateriaisBrut = (materialRepo.findByDisciplina ? await materialRepo.findByDisciplina(discId) : [])
      || [];
    ...
    const trilhas = (await trilhaRepo.findByDisciplina(discId)||[]).map(async t => {
    ...
    try {
      if (req.user.perfil === 'aluno') {
        const turmaIds = (await turmaRepo.getTurmasAluno(req.user.id)).map(m => m.turma_id);
        avaliacoes = (await Promise.all(turmaIds.map(tid => avaliacaoRepo.findByTurma(tid)))).flat()
          .filter(av => av.disciplina_id === discId || !av.disciplina_id)
          .filter(av => av.status === 'publicada');
      } else {
        avaliacoes = (avaliacaoRepo.findByDisciplina ? await avaliacaoRepo.findByDisciplina(discId) : []) || [];
      }
    } catch(e) { avaliacoes = []; }
    ...
    let atividades = [];
    try {
      atividades = (atividadeRepo.findByDisciplina ? await atividadeRepo.findByDisciplina(discId)
        : atividadeRepo.findAll ? (await atividadeRepo.findAll()).filter(a => a.disciplina_id === discId)
        : []) || [];
      if (req.user.perfil === 'aluno') {
    ...
    // Turmas vinculadas
    const turmasVinculadas = (await tdRepo.findByDisciplina(discId)||[]).map(async td => {
      const t = await turmaRepo.findById(td.turma_id);
