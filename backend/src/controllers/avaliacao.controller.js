...
async function list(req, res, next) {
  try {
    ...
    if (req.user.perfil === 'aluno') {
      ...
      avs = await avaliacaoRepo.findByDisciplina(disciplina_id);
    } else if (turma_id) {
      avs = await avaliacaoRepo.findByTurma(turma_id);
    } else if (req.user.perfil === 'professor') {
      avs = await avaliacaoRepo.findByProfessor(req.user.id);
    } else {
      avs = await avaliacaoRepo.findAll();
    }
...
async function getById(req, res, next) {
  try {
    ...
    const questoes = (av.questoes || []).map(async qc => {
    ...
    const tentativas = req.user.perfil === 'aluno'
      ? await avaliacaoRepo.findTentativaAlunoAvalia(req.user.id, av.id)
      : [];
...
async function create(req, res, next) {
  try {
    ...
    const av = await avaliacaoRepo.create({
...
async function publicar(req, res, next) {
  try {
    ...
    const updated = await avaliacaoRepo.update(req.params.id, { status: 'publicada' });
...
async function iniciar(req, res, next) {
  try {
    ...
    const tentativas = await avaliacaoRepo.findTentativaAlunoAvalia(req.user.id, av.id);
...
async function responderQuestao(req, res, next) {
  try {
    ...
    const tentativa = await avaliacaoRepo.findTentativaById(tentativa_id);
    ...
    await avaliacaoRepo.updateTentativa(tentativa_id, { respostas });
...
async function concluir(req, res, next) {
  try {
    ...
    if (req.body.respostas && Array.isArray(req.body.respostas) && req.body.respostas.length > 0) {
      ...
      await avaliacaoRepo.updateTentativa(tentativa.id, { respostas: currentResps });
    ...
    const turma        = av.turma_id ? await turmaRepo.findById(av.turma_id) : null;
    const disc         = av.disciplina_id ? require('../repositories/disciplina.repository').findById(av.disciplina_id) : null;
    const numTentativa = (await avaliacaoRepo.findTentativasByAvalia(av.id)).filter(t => t.aluno_id === req.user.id && t.status === 'concluida').length;
...
async function resultados(req, res, next) {
  try {
    const av = await avaliacaoRepo.findById(req.params.id);
    if (!av) return res.status(404).json({ error: 'Avaliação não encontrada.' });
    const tentativas = (await avaliacaoRepo.findTentativasByAvalia(av.id)).filter(t => t.status === 'concluida');
    const porAluno = {};
    for (const t of tentativas) {
      if (!porAluno[t.aluno_id]) porAluno[t.aluno_id] = [];
      porAluno[t.aluno_id].push(t);
    }
    const stats = await Promise.all(Object.entries(porAluno).map(async ([uid, ts]) => {
      const user = await userRepo.findById(Number(uid));
      if (!user) return null;
    ...
    const statsFilt = stats.filter(Boolean).sort((a,b) => b.melhor_nota - a.melhor_nota);
    const notas = statsFilt.map(s => s.melhor_nota);
    const media = notas.length > 0 ? Math.round(notas.reduce((a,b)=>a+b,0)/notas.length*100)/100 : 0;
    res.json({ avaliacao: av, resultados: statsFilt, estatisticas: { total_alunos: stats.length, media_geral: media, aprovados: stats.filter(s=>s.aprovado).length, reprovados: stats.filter(s=>!s.aprovado).length, taxa_aprovacao: stats.length > 0 ? Math.round(stats.filter(s=>s.aprovado).length/stats.length*100) : 0 } });
  } catch(e){ next(e); }
}
...
async function clonar(req, res, next) {
  try {
    ...
    const clone = await avaliacaoRepo.create({
      ...av,
      id: undefined,
...
async function vincularTurmas(req, res, next) {
  try {
    ...
    if (turma_ids.length === 1) await avaliacaoRepo.update(av.id, { turma_id: Number(turma_ids[0]) });
...
async function analiseTurma(req, res, next) {
  try {
    ...
    const questaoRepo = require('../repositories/questao.repository');
    const todasTentativas = (await avaliacaoRepo.findTentativasByAvalia(av.id)).filter(t => t.status === 'concluida');
    const questoesConfig  = av.questoes || [];
...
async function detalheAluno(req, res, next) {
  try {
    ...
    const tentativas = await avaliacaoRepo.findTentativasByAvalia(av.id)
    ...
    const respostasCompletas = await Promise.all(respsBase.map(async function(rc) {
      const q   = await questaoRepo.findById(rc.questao_id);
...
async function listarEntregas(req, res, next) {
  try {
    ...
    const questoesUpload = (av.questoes || [])
      .map(async qc => await questaoRepo.findById(qc.questao_id))
...
async function corrigirManual(req, res, next) {
  try {
    ...
    await avaliacaoRepo.updateTentativa(tentativa.id, { respostas, nota: novaNota, aprovado });
