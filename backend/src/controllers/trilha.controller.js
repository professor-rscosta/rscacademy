...
async function list(req, res, next) {
  try {
    ...
    if (req.user.perfil === 'aluno') {
      ...
      trilhas = discIds.flatMap(did => trilhaRepo.findByDisciplina(did));
    } else if (disciplina_id) {
      trilhas = trilhaRepo.findByDisciplina(disciplina_id);
    } else if (professor_id) {
      trilhas = trilhaRepo.findByProfessor(professor_id);
    } else if (req.user.perfil === 'professor') {
      trilhas = trilhaRepo.findByProfessor(req.user.id);
    } else {
      trilhas = trilhaRepo.findAll();
    }
...
async function getById(req, res, next) {
  try {
    const t = trilhaRepo.findById(req.params.id);
    ...
    const questoes = questaoRepo.findByTrilha(t.id);
...
async function update(req, res, next) {
  try {
    const t = trilhaRepo.update(req.params.id, req.body);
    if (!t) return res.status(404).json({ error: 'Trilha não encontrada.' });
    res.json({ trilha: t });
  } catch(e){ next(e); }
}
...
async function remove(req, res, next) {
  try { trilhaRepo.remove(req.params.id); res.json({ message: 'Trilha removida.' }); }
  catch(e){ next(e); }
}
