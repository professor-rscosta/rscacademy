...
async function submitResposta(req, res, next) {
  try {
    ...
    const questao = questaoRepo.findById(questao_id);
    ...
    const respostasAnteriores = respostaRepo.findByAluno(aluno_id);
    ...
    // ── Salvar resposta ────────────────────────────────────────
    const resposta_salva = respostaRepo.create({
    ...
    dbUpdate('usuarios', aluno_id, { theta: theta_depois, xp_total: (req.user.xp_total || 0) + xp_ganho });

    // ── Atualizar contador de respostas da questão ─────────────
    const totalRespostas = respostaRepo.countByQuestao(questao_id);
    questaoRepo.updateTRI(questao_id, { ...questao.tri, total_respostas: totalRespostas });
...
async function listByTrilha(req, res, next) {
  try {
    ...
    const respostas = respostaRepo.findByAlunoTrilha(req.user.id, trilha_id);
...
async function tentativasTrilha(req, res, next) {
  try {
    ...
    const respostas = respostaRepo.findByAluno(aluno_id).filter(r => {
      const q = questaoRepo.findById(r.questao_id);
