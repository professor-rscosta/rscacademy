...
async function getById(req, res, next) {
  try {
    const q = await questaoRepo.findById(req.params.id);
...
async function create(req, res, next) {
  try {
    ...
    let trilha = null;
    if (trilha_id) {
      trilha = await trilhaRepo.findById(trilha_id);
...
async function update(req, res, next) {
  try {
    ...
    const q = await questaoRepo.update(req.params.id, fields);
    if (!q) return res.status(404).json({ error: 'Questão não encontrada.' });
    res.json({ questao: q });
  } catch (err) { next(err); }
}
...
async function remove(req, res, next) {
  try {
    await questaoRepo.remove(req.params.id);
