...
const qualquerAutenticado = (req, res, next) => next(); // só precisa do JWT

// ── 4. Professor só gerencia suas próprias turmas ─────────────
function donoOuAdmin(req, res, next) {
  if (req.user.perfil === 'admin') return next();
  const turma = turmaRepo.findById(req.params.id || req.params.turma_id);
