...
const CATEGORIES = [
  {
    ...
    actions: [
      { id:'disciplinas', icon:'📚', label:'Disciplinas', desc:'Criar e configurar disciplinas com materiais' },
      { id:'materiais',   icon:'📂', label:'Hub de Aprendizagem', desc:'Postar PDFs, vídeos e links para alunos' },
      { id:'trilhas',     icon:'🗺️', label:'Trilhas Gamificadas', desc:'Criar trilhas de aprendizagem com XP e TRI' },
      { id:'questoes',    icon:'❓', label:'Banco de Questões', desc:'Gerenciar questões com IA e correção automática' },
    ],
  },
  {
    id: 'avaliacao',
    label: '📝 Avaliação & Atividades',
    color: '#0ea5e9',
    bg: 'rgba(14,165,233,.06)',
    border: 'rgba(14,165,233,.2)',
    actions: [
      { id:'avaliacoes', icon:'📝', label:'Avaliações', desc:'Provas, simulados e quizzes com correção automática' },
      { id:'atividades', icon:'📋', label:'Atividades', desc:'Envio de arquivos e entregas dos alunos' },
      { id:'boletim',    icon:'📊', label:'Boletim',    desc:'Notas e desempenho por aluno e disciplina' },
      { id:'relatorios', icon:'📈', label:'Relatórios', desc:'Análise completa de desempenho da turma' },
    ],
  },
  {
    id: 'turma',
    label: '🏫 Turmas & Comunicação',
    color: '#10b981',
    bg: 'rgba(16,185,129,.06)',
    border: 'rgba(16,185,129,.2)',
    actions: [
      { id:'turmas',   icon:'🏫', label:'Turmas', desc:'Criar turmas e matricular alunos' },
      { id:'mural',    icon:'📌', label:'Mural de Avisos', desc:'Publicar avisos e comunicados para a turma' },
      { id:'chatbot',  icon:'🤖', label:'Assistente IA', desc:'Configure o assistente Lumi para seus alunos' },
    ],
  },
];
...
export default function ProfHome({ onNavigate }) {
  ...
  useEffect(() => {
    Promise.all([
      api.get('/turmas?professor_id=' + user?.id).catch(() => ({ data: { turmas: [] } })),
      api.get('/disciplinas?professor_id=' + user?.id).catch(() => ({ data: { disciplinas: [] } })),
      api.get('/trilhas?professor_id=' + user?.id).catch(() => ({ data: { trilhas: [] } })),
      api.get('/questoes?professor_id=' + user?.id).catch(() => ({ data: { questoes: [] } })),
      api.get('/avaliacoes?professor_id=' + user?.id).catch(() => ({ data: { avaliacoes: [] } })),
    ]).then(([t, d, tr, q, av]) => {
      const turmas = t.data.turmas || [];
      const alunos = turmas.reduce((acc, t) => acc + (t.total_alunos || 0), 0);
      setStats({
    ...
    <div style={{ maxWidth:1100, margin:'0 auto', padding:'0 0 2rem' }}>
      ...
      <div style={{
        ...
        <div style={{ marginBottom: '1.5rem' }}>
          ...
          <p style={{ fontSize: 13, color: 'rgba(255,255,255,.45)', margin: 0 }}>
            Você possui <strong style={{ color: '#10b981' }}>{stats.turmas} turmas ativas</strong> e <strong style={{ color: '#10b981' }}>{stats.alunos} alunos matriculados</strong>.
          </p>
        </div>
        {/* Stats row below greeting */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(110px, 1fr))', gap: 10 }}>
          {[
            ...
            { icon:'🏫', label:'Turmas',      val: stats.turmas,       color:'#f472b6' },
            { icon:'👨‍🎓', label:'Alunos',     val: stats.alunos,       color:'#4ade80' },
          ].map(s => (
      ...
      {CATEGORIES.map(cat => (
        <div key={cat.id} style={{
          ...
          <div style={{ fontSize: 14, fontWeight: 700, color: cat.color, marginBottom: '1rem', letterSpacing: '-.2px' }}>
            {cat.label}
          </div>
